<template>
  <div>
    <el-tag v-if="isSetNode" v-for="(item,index) in nodeList" :key="index" closable @click.native="upDateNode(item,index)" @close="closeNodeList(index)">打卡点{{index+1}}:{{item.nodeName}}</el-tag>
